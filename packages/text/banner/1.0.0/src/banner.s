;
; cl65 -o BANNER-FONT.BIN -t cx16 -C cx16-asm.cfg banner.s
;
chrout		= $ffd2
...
putspace:
		jsr twospaces	; someone wants a space char
		jsr down
		jsr twoleft
		jsr twospaces
		jsr down
		jsr twoleft
		jsr twospaces
		bra done        ; mission accomplished (used to be a JMP)

		;
		; Char to print is POKEd into R0L ($02).
...
putchar:	
		...
		; Yes, we overwrite R0L but its value is in x now.
		;
		lda table_lo,x
		sta $02
		lda table_hi,x
